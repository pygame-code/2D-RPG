...
class player(object):
    ...
    def __init__(self, x, y, width, height, class_index, player_class, health, stamina, mana, abilities, base_dmg, dodge_chance, armour):
        ...
        self.crit_dmg_mult = 1.3 # crit damage multiplier
        self.dodge_chance = dodge_chance
        self.armour = armour

        self.base_attack_loop = False
    ...
    def pressed(self, key): # Let the player hotkey their abilities to different buttons
        ...
        if key == 1:
            self.useAbilityOnEnemy(self.abilities[4]) # Starting class-specific move

    def useAbilityOnEnemy(self, ability):
        ...
        if enemy.any_highlighted:
            ...
            if ability == 'Base Attack':
                ...
                if self.class_index == 1 or self.class_index == 2: # melee
                    ...
                    vect = rel_enemy.distanceFromPlayer(dist_only = True)
                    if vect <= np.sqrt(((self.width/2) + (rel_enemy.widths[0]/2) + 20)**2 + ((self.height/2) + (rel_enemy.heights[0]/2) + 20)**2):

                        if not self.base_attack_loop:

                            if self.initialAttack: # Attack initially the instant the enemy is within distance
                                self.baseAttack(rel_enemy)
                                self.initialAttack = False
                            else:
                                Timer(2, self.baseAttack, [rel_enemy]).start() # Calls the function in 2 seconds.

                            self.base_attack_loop = not self.base_attack_loop # Acts as a switch to turn off calling the attack function until 2 seconds have passed.
    ...
    def useAbilityOnSelf(self):
        pass

    def baseAttack(self, enemy_obj):

        dodge_rand = np.random.rand()
        dmg_rand_int = np.random.randint(-3, 4) # Int between -3 and 3
        crit_rand = np.random.rand()

        vect = enemy_obj.distanceFromPlayer(dist_only = True)
        if vect <= np.sqrt(((self.width/2) + (enemy_obj.widths[0]/2) + 20)**2 + ((self.height/2) + (enemy_obj.heights[0]/2) + 20)**2):

            if enemy_obj.health > 0:
                
                if dodge_rand > enemy_obj.dodge_chance: # If enemy does not dodge...
                    if crit_rand > self.crit_chance: # If not a critical hit...
                        enemy_obj.health -= self.base_dmg + int(round(self.base_dmg * dmg_rand_int / 10)) # plus or minus up to 30% of base dmg
                        enemy_obj.health = max(0, enemy_obj.health)
                        # Add floating damage text above enemy's head

                    else: # If a critical hit...
                        enemy_obj.health -= int(round((self.base_dmg + (self.base_dmg * dmg_rand_int / 10)) * self.crit_dmg_mult))
                        enemy_obj.health = max(0, enemy_obj.health)
        ...
        self.base_attack_loop = not self.base_attack_loop
    ...
    def enemyAggro(self, enemy_obj):

        if not enemy_obj.attackLoop: # If the enemy is not currently chasing you, they will now that you have attacked them.
            if enemy_obj.threat == 1 or enemy_obj.threat == 2: # If the enemy is not passive
                enemy_obj.attackLoop = True 
...
class enemy(object):
    ...
    objs = [] # Registrar
    any_highlighted = False # True if any enemy is already highlighted.
    ...
    def __init__(self, x, y, widths, heights, vel, containmentArea, health, convert12to16, threat):
        ...
        self.exp_given = (self.level)**2 # Change this at some point
        self.base_dmg = 5 * self.level
        self.dodge_chance = 0.03
        self.crit_chance = 0.05
        self.crit_dmg_mult = 1.3
        ...
        self.alive = True # Make the enemy disappear from the screen if this is false (still loaded but it's not drawn and it can't attack).
    ...
    def killed(cls):
        ...
        for obj in cls.objs:
            if obj.health == 0:
                if obj.alive:
                    man.gainExp(obj.exp_given, expThreshold)
                    '''
                    Drop an item or gold at the enemy's last location. And show it on the screen.
                    Have the player walk over the item/gold to pick it up.
                    The location of death can be a parameter passed to this method.
                    '''
                    obj.alive = False
    ...
    def distanceFromPlayer(self, dist_only = False):
...
while run:
    ...
    for dist in distances:
        ...
        if dist <= obj.aggro_dist and obj.threat == 1 and not obj.attackLoop:
            obj.attackLoop = True
            if not enemy.any_highlighted: # If no enemies are highlighted yet...
                enemy.highlightEnemy(distances.index(dist))
    ...
    man.useAbilityOnEnemy('Base Attack')
